...
class StreamReassembler {
  private:
    ...
    struct blockNode{
      ...
      string data="";
      bool operator < (const blockNode block) {return indexBegin < block.indexBegin;}
      blockNode() = default;
      blockNode(uint64_t index, uint64_t len, const string &data): indexBegin(index),streamLength(len),data(data){}
    };
    set<blockNode> assembler{};
    size_t _remaining_capacity; // reassembler stream remaining capacity
    uint64_t _nexindex=0; // next index of last byte in _output
  ...
  public:
    ...
    bool merge_block(blockNode &b1,const blockNode &b2);
